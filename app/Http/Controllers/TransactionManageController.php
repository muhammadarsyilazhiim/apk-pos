...
class TransactionManageController extends Controller
{
    ...
    public function viewTransaction()
    {
        ...
        if ($check_access->transaksi == 1) {
            ...
            return view('transaction.transaction', compact('products', 'supply_system', 'market', 'customers'));
        } else {
            return back();
        }
    }
    ...
    public function transactionProcess(Request $req)
    {
        ...
        if ($check_access->transaksi == 1) {
            ...
            for ($i = 0; $i < $jml_barang; $i++) {
                ...
                $transaction->harga_beli = $product_data->harga_beli;
                $transaction->total_harga_beli = $product_data->harga_beli * $req->jumlah_barang[$i];
                // dd($product_data->harga_beli);
                ...
                $transaction->id_customer = $req->id_customer;
                $transaction->metodePem = $req->metodePem;
    ...
    public function receiptTransaction($id)
    {
        ...
        ->first();
        if($check_access->transaksi == 1){
            $transaction = Transaction::with('customer')->where('transactions.kode_transaksi', '=', $id)
            ->select('transactions.*')
            ->first();
            $transactions = Transaction::with('customer')->where('transactions.kode_transaksi', '=', $id)
            ->select('transactions.*')
            ->get();
